//! Generated deterministic NFC tables and implementation.
//!
//! Unicode data version: 15.1.0. This file is generated from the Unicode Character
//! Database and committed so GVYA normalization does not depend on host libraries,
//! locale, platform, or a crates.io dependency at runtime/build time.
...
fn decompose(cp: u32, out: &mut Vec<u32>) {
    ...
    if let Ok(index) = DECOMP.binary_search_by_key(&cp, |row| row.0) {
        let (_, first, second) = DECOMP[index];
        decompose(first, out);
        if second != 0 {
...
fn combining_class(cp: u32) -> u8 {
    CCC.binary_search_by_key(&cp, |row| row.0)
        .map_or(0, |index| CCC[index].1)
}
...
fn compose(values: &[u32]) -> String {
    let mut out: Vec<u32> = Vec::with_capacity(values.len());
    let mut starter_index: Option<usize> = None;
    let mut starter = 0_u32;
    let mut last_class = 0_u8;
    for &cp in values {
        let class = combining_class(cp);
        let composite = starter_index.and_then(|_| compose_pair(starter, cp));
        if let (Some(index), Some(composed)) = (starter_index, composite) {
    ...
    out.into_iter()
        .map(|cp| char::from_u32(cp).expect("generated NFC tables contain valid Unicode scalars"))
        .collect()
}
...
fn compose_pair(first: u32, second: u32) -> Option<u32> {
    ...
    COMPOSE
        .binary_search_by(|row| (row.0, row.1).cmp(&(first, second)))
        .ok()
...
mod tests {
    use super::nfc;
    #[test]
    fn nfc_handles_canonical_equivalence_and_hangul() {
